...
int GetPrime(mp_int *m,int lon)
{
	...
	ret = mp_prime_random_ex(m, 10, lon, 
		(rand()&1)?0:LTM_PRIME_2MSB_ON, myrng, NULL);
	return ret;
}
					  ...
					  mp_int * mp_XA, mp_int * mp_YA)
{
...
#ifdef  WIN32
	sprintf_s(tmp, sizeof(tmp), "%4lx", lenUID * 8);
#else
	...
	memset(ZA_SRC_Buff, 0x00 , lenZA_SRC + MAX_STRLEN);
...
int KDFwithSm3(unsigned char * kdfOutBuff, unsigned char * Z_in, unsigned long ulZlen, unsigned long klen )
{
	...
	unsigned char * pZandCt = (unsigned char*)malloc(ulZlen + 4 + 10);
	if (NULL == pZandCt)
	{
		return ERR_MEM_ALLOC;
	}
	...
	{
#ifdef WIN32
		sprintf_s(ct_str, sizeof (ct_str), "%8lx", ct);
#else
